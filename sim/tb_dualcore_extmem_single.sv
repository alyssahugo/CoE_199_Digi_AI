...
module tb_dualcore_extmem_single();
    ...
    localparam string temp_inst = $sformatf("%s%s%s", `REPO_LOCATION, `TEST_LOCATION, "instmem-dump/mem/shared_counter_amoadd_l.mem");
    // localparam string temp_data = $sformatf("%s%s%s", `REPO_LOCATION, `TEST_LOCATION, "datamem-dump/mem/program_data.hex");
    localparam string temp_data = $sformatf("%s%s%s", `REPO_LOCATION, `TEST_LOCATION, "datamem-dump/mem/race_conditions_data.mem");
    localparam string temp_refm = $sformatf("%s%s%s", `REPO_LOCATION, `TEST_LOCATION, "answer-keys/mem/race_conditions_data.mem");
    ...
    wire active_data_op_0 = ext_cache_req || ext_noncache_req_0;
    wire active_data_op_1 = ext_cache_req || ext_noncache_req_1;
    /*
    datamem DATAMEM (
        .clk(CLK),
        .nrst(nrst),
    ...
    */
    ...
    wire [31:0] bram_dinA;
    wire [31:0] bram_dinB;
   bd_dual_testbench_wrapper rv32_dual (
	...
	integer i, j, check_0, check_1, done, pass, consecutive_nops;
	...
	initial begin
        ...
        $readmemh(temp_inst, INSTMEM_0.instmem);
         $readmemh(temp_inst, INSTMEM_1.instmem);
	...
	always@(posedge CLK) begin
		if(check_0 == 50 || consecutive_nops == 25) done = 1;
	end
