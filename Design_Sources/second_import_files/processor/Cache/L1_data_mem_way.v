...
module L1_data_mem_way
    ...
    // generate four columns
        for (i = 0; i < 4; i = i + 1) begin: data_column
            data_mem_column #(.INDEX_BITS(INDEX_BITS))
                data_column (.clk(clk), .i_index(i_index), .i_weA(we_bus[i] && i_wr && i_hit && !i_data_from_mem_valid), 
                    ...
                    .i_data_from_mem(data_from_mem_n[i]), .i_weB(i_hit && i_data_from_mem_valid && !i_wr),
                    .i_data_from_core(i_data_from_core), .o_data(output_column[i]));
        end
    endgenerate
    
    //assign o_out_way = {output_column[3], output_column[2], output_column[1], output_column[0]};
