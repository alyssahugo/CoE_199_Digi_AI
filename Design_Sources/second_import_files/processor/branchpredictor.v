...
*/
	...
	wire [`BHT_ENTRY_BITS-1:0] exe_loadentry = {exe_PC[`BHT_SET_BITS-1:0], 2'b00};
	assign exe_PBT = exe_branchtarget;
	assign exe_CNI = {exe_PC} + ((|exe_c_btype)? 2'd1 : 2'd2);
	
	// is_pred_correct: determines if the prediction is correct
	// Check if prediction is correct & output appropriate correction
	// If sat_counter[1] and feedback are equal, then prediction is correct.
	// Else, prediction is wrong and correction output needs to be set.
	wire is_pred_correct = 1'b0;
...
*/
	...
	always@(*) begin
		if(!stall) begin
			if( ((|exe_btype || |exe_c_btype) && !is_pred_correct) || (exe_sel_opBR) )
				branch_flush_t = 1;
			else
				branch_flush_t = 0;
		end else
