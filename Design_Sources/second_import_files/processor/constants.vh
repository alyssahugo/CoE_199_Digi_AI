...
`define MEM_DEPTH 65536          // Number of words in instruction memory
`define ISR_DEPTH 512           // ISR
`define MEM_WIDTH 32            
`define WORD_WIDTH 32           // Word width of 32bits; Used for Instructions, operands, and immediates

`define PC_ADDR_BITS 17         
`define EXT_PC_ADDR_BITS 17     // External address width
...
`define NUM_SETS  (`NUM_BLOCKS / `CACHE_WAY)
`define L2_NUM_SETS (`L2_NUM_BLOCKS / `L2_CACHE_WAY)
`define INDEX_BITS  $clog2(`NUM_SETS)
`define L2_INDEX_BITS $clog2(`L2_NUM_SETS)
`define TAG_BITS  (`ADDR_BITS - `INDEX_BITS - `OFFSET_BITS - 2)
`define L2_TAG_BITS (`ADDR_BITS - `L2_INDEX_BITS - `OFFSET_BITS - 2)
