...
###############################################################################
# Minimal XDC to pass bitstream DRC for your current top-level ports
# Fixes: NSTD-1 + UCIO-1 for init_calib_complete_0, UART_0_rxd, UART_0_txd
###############################################################################

## -----------------------------
## SYSTEM CLOCK (confirmed)
## -----------------------------
set_property -dict { PACKAGE_PIN R4 IOSTANDARD LVCMOS33 } [get_ports sys_clock]

# NOTE:
# If you are using clk_wiz_0, Vivado usually already constrains sys_clock via the
# clk_wiz generated XDC. Adding a create_clock here may cause an "overrides clock"
# warning. That's OK, but to keep things clean, we do NOT create_clock here.
#
# If you are NOT using clk_wiz and need a primary clock constraint, uncomment:
# create_clock -name sys_clk -period 10.000 [get_ports sys_clock]   ;# 100 MHz
# create_clock -name sys_clk -period 20.000 [get_ports sys_clock]   ;# 50 MHz


## -----------------------------
## RESET (you used this already)
## -----------------------------
# Keep this if nrst is actually connected to that pin on your board.
# If nrst comes from a switch/button bank at 1.2V or 1.5V, you MUST match that IOSTANDARD.
set_property -dict { PACKAGE_PIN E22 IOSTANDARD LVCMOS33 } [get_ports nrst]


## -----------------------------
## UART (FIXED PORT NAMES)
## -----------------------------
# These must match your top-level ports exactly:
# get_ports * shows UART_0_rxd and UART_0_txd
set_property -dict { PACKAGE_PIN AA19 IOSTANDARD LVCMOS33 } [get_ports UART_0_rxd]
set_property -dict { PACKAGE_PIN V18  IOSTANDARD LVCMOS33 } [get_ports UART_0_txd]


## -----------------------------
## MIG calibration done (was blocking bitstream)
## -----------------------------
# You currently have init_calib_complete_0 as an external top-level port.
# To satisfy DRC, we must assign it to a real pin + IOSTANDARD.
#
# Easiest: drive an LED.
# Nexys Video LED[0] is commonly T14 with LVCMOS25 in the Digilent master XDC.
set_property -dict { PACKAGE_PIN T14 IOSTANDARD LVCMOS25 } [get_ports init_calib_complete_0]


## -----------------------------
## Configuration (safe to keep)
## -----------------------------
set_property CONFIG_VOLTAGE 3.3 [current_design]
set_property CFGBVS VCCO [current_design]
###############################################################################